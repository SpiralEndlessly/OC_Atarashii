...
if opts.print_options:
    ...
    print()
    print(
        "ONE_CHANGE: if True, shows the image after each iteration (even if no difference)"
    )
    print(
        "\t else, shows the image only if there is a difference with *ALL* the previous images"
    )
    print()
    print(
        "COMPARE_WITH_PREVIOUS: if True, updates the base image and ram_n after each display"
    )
