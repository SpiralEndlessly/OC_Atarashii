...
from ocatarashii.vision.tennis import objects_colors
from ocatarashii.vision.pong import objects_colors
from ocatarashii.vision.bowling import objects_colors
from ocatarashii.vision.breakout import objects_colors
from ocatarashii.utils import load_agent, test_parser, make_deterministic, RandomAgent
...
print(
    colored(
        f"Using {MIN_ACCEPTABLE_IOU} as iou threshold for the saved images..", "magenta"
    )
)
...
if ONLYBOTHDEFINEDOBJECTS:
    ...
    for mode in ["ram", "vision"]:
        mod_path = f"ocatarashii.{mode}.{game_name.lower()}"
        classes.append(
            [el[0] for el in inspect.getmembers(sys.modules[mod_path], inspect.isclass)]
        )
...
for agent, meth_name in zip(agents, ["Random", "DQN", "C51"]):
    if agent is not None:
                ...
                if i % 20 == 0:
                    if ONLYBOTHDEFINEDOBJECTS:
                        robjects = [
                            ...
                            if e.__class__.__name__ in classes_in_both
                        ]
                        vobjects = [
                            e
                            for e in env.objects_v
                            if e.__class__.__name__ in classes_in_both
                        ]
                    else:
                    ...
                    if stats["mean_iou"] < MIN_ACCEPTABLE_IOU:
                        ...
                        for ax, obs, objects_list, title in zip(
                            axes, [obse, obse2], [robjects, vobjects], ["ram", "vision"]
                        ):
                        ...
                        plt.tight_layout()
                        plt.savefig(
                            f"{SAVE_IMAGE_FOLDER}/{game_name}_{meth_name}_{image_n}.png"
                        )
                        # plt.show()
                        # print(env.objects)
                        # print(env.objects_v)
        ...
        json_report_bad = (
            f"Details of frames with mean iou < {MIN_ACCEPTABLE_IOU}\n"
            + json_report_bad
        )
        with open(
            f"{SAVE_FOLDER}/report_bad_{game_name}_{meth_name}.json", "w"
        ) as outfile:
            outfile.write(json_report_bad)
        json_all_stats = json.dumps(ALL_STATS, indent=4)
        with open(
            f"{SAVE_FOLDER}/all_stats_{game_name}_{meth_name}.json", "w"
        ) as outfile:
            outfile.write(json_all_stats)

        print_all_stats(ALL_STATS)
        print(
            f"Saved report_bad_{game_name}_{meth_name}.json and all_stats_{game_name}_{meth_name}.json in {SAVE_FOLDER}"
        )

        if im_reports:
            print(
                f"Saved the following images with iou < {MIN_ACCEPTABLE_IOU}:\n"
                + im_reports
                + f"\n in {SAVE_IMAGE_FOLDER}"
            )
            print(
                f"Saved {SAVE_FOLDER}/report_bad_{game_name}_{meth_name}.json for details on these images"
            )
        ...
        dfs.append(
            pd.DataFrame.from_dict(
                format_values(det_scores.cat_precisions),
                orient="index",
                columns=["Precision"],
            )
        )
        dfs.append(
            pd.DataFrame.from_dict(
                format_values(det_scores.cat_recalls),
                orient="index",
                columns=["Recall"],
            )
        )
        dfs.append(
            pd.DataFrame.from_dict(
                format_values(det_scores.cat_f_scores),
                orient="index",
                columns=["F-score"],
            )
        )
        dfs.append(
            pd.DataFrame.from_dict(format_values(ious), orient="index", columns=["IOU"])
        )
        df_finals.append(
            ft.reduce(lambda left, right: pd.DataFrame.join(left, right), dfs)
        )
        ...
        df_finals.append(
            pd.DataFrame(columns=["Precision", "Recall", "F-score", "IOU"])
        )
...
ltx_code = ltx_code.replace("lrrrrrrrrrrr", "l|rrrr|rrrr|rrrr").replace(
    "multicolumn{4}{c}{Random}", "multicolumn{4}{c|}{Random}"
)
...
for det_scores, agent in zip(detections_scores, agents):
    if agent is not None:
        base_dfs.append(
            pd.DataFrame.from_dict(
                det_scores.dict_summary, orient="index", columns=[opts.game]
            ).T
            * 100
        )
    else:
