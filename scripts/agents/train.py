...
from ocatarashii.environments import PositionHistoryEnv
...
from ocatarashii.utils import parser
...
class RtptCallback(BaseCallback):
    def __init__(self, exp_name, max_iter, verbose=0):
        ...
        self.rtpt = RTPT(
            name_initials="QD", experiment_name=exp_name, max_iterations=max_iter
        )
        self.rtpt.start()
...
def main():
    parser.add_argument(
        "-g", "--game", type=str, required=True, help="game to train (e.g. 'Pong')"
    )
    parser.add_argument("-s", "--seed", type=int, required=True, help="seed")
    parser.add_argument(
        "-c", "--cores", type=int, required=True, help="number of envs used"
    )
    ...
    if opts.rgb:
        ...
        eval_env = SubprocVecEnv(
            [
                make_env(game=env_str, rank=i, seed=eval_env_seed)
                for i in range(n_eval_envs)
            ],
            start_method="fork",
        )
        env = SubprocVecEnv(
            [make_env(game=env_str, rank=i, seed=opts.seed) for i in range(n_envs)],
            start_method="fork",
        )
    ...
    rtpt_callback = RtptCallback(exp_name=exp_name, max_iter=rtpt_iters)

    n_callback = EveryNTimesteps(n_steps=rtpt_frequency, callback=rtpt_callback)
