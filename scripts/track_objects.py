...
from ocatarashii.utils import make_deterministic
import atexit
...
class Renderer:
    ...
    def __init__(self, env_name: str, mode: str, hud: bool):
        ...
        self.slots_imgs = [
            np.zeros((100, 100, 3), dtype=np.uint8) for _ in range(self.max_objects)
        ]
    ...
    def _init_pygame(self, sample_image):
        ...
        self.window_size = (
            self.env_render_shape[0] + object_render_width,
    ...
    def _render_objects(self):
        if not self.both:
            ...
            objects = [
                item
                for pair in zip(self.env.objects, self.env.objects_v)
                for item in pair
            ]
            num_objects = min(len(objects), self.max_objects)
            # if len(objects) > self.max_objects:
    ...
    def _render_object_cell(self, cell_idx, obj):
        ...
        if obj:
            ...
            if self.track_color_change and obj_init_color[:3] != border_color:
                pygame.draw.rect(
                    self.window, (255, 20, 20), [x - 2, y - 2, w + 4, h + 4], 8
                )
                self.paused = True
                print(
    ...
    def _get_obj_sprite(self, obj):
        if not obj:
            return (
                np.ones(
                    (
                        round(OBJ_CELL_WIDTH * (UPSCALE_FACTOR / 4)),
                        round(OBJ_CELL_HEIGHT * (UPSCALE_FACTOR / 4)),
                        3,
                    ),
                    dtype=np.uint8,
                )
        ...
        try:
            padded_image[
                top_left_y : top_left_y + h, top_left_x : top_left_x + w, :
            ] = sprite
        except ValueError:
            import ipdb
    ...
    def _get_obj_cell_rect(self, idx: int):
        ...
        x = self.obj_grid_anchor_left + round(
            col * (OBJ_CELL_WIDTH + 5) * (UPSCALE_FACTOR / 4)
        )
        y = self.obj_grid_anchor_top + round(
            row * (OBJ_CELL_HEIGHT + 5) * (UPSCALE_FACTOR / 4)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-hud", "--hud", action="store_true", help="Display HUD")
    parser.add_argument("-ls", "--load_state", type=str, default="")
    parser.add_argument(
        "-m", "--mode", type=str, default="ram", help="Detection mode to be run"
    )
