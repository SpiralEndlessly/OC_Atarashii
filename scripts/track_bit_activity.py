...
def bit_correlation(bit_mats, tresh):
    ...
    plt.colorbar(
        mappable=img, ax=ax, orientation="vertical", fraction=0.1, label="Correlation"
    )
...
print(f"Saved {len(ram_saves['no_obj'])} frames with {opts.tracked_objects} absent")
...
def plot_bit_activity(bit_mats_condensed, title):
    ...
    fig, ax = plt.subplots()
    img = plt.imshow(hist_mat, aspect="auto", cmap="viridis", interpolation="none")

    ax.set_xticks(np.arange(0, hist_mat.shape[1], 1.0), labels=active_positions)
...
def compare_bit_mats(obj_saves, no_obj_saves):
    ...
    hist_mat_obj = np.sum(np.abs(d_object_mats), axis=0) / d_object_mats.shape[0]
    hist_mat_no_obj = (
        np.sum(np.abs(d_no_object_mats), axis=0) / d_no_object_mats.shape[0]
    )
    ...
    titles = (
        "Tracked object absent",
        "Tracked object present",
        "Masked out inactive bits in 'absent' condition",
    )
    fig, axs = plt.subplots(3, 1)

    for row, data, title in zip(axs, data, titles):
        img = row.imshow(data, aspect="auto", cmap="viridis", interpolation="none")
        row.set_xticks(np.arange(0, data.shape[1], 1.0), labels=active_positions)
    ...
    fig.suptitle(
        f"Comparison of bitwise activity with respect to {opts.tracked_objects}",
        fontsize=20,
    )
    fig.tight_layout()
...
class IndexTracker:
    def __init__(self, axs, bit_mats, game_imgs):
        ...
        a.set_xticks(np.arange(0, len(self.ram_labels), 1.0), labels=self.ram_labels)
        a.tick_params(axis="x", labelrotation=90, labelsize=6)
        plt.colorbar(mappable=self.im, ax=a, orientation="horizontal", fraction=0.025)
        ...
        d = axs["DIFF_IMG"]
        self.im5 = d.imshow(
            self.game_imgs[self.index + 1, :, :, :]
            - self.game_imgs[self.index, :, :, :]
        )
...
if opts.interactive:
    ARR = [["BITMAP", "BITMAP", "BITMAP"], ["PREV_IMG", "CURR_IMG", "DIFF_IMG"]]
