...
for i in tqdm(range(NB_SAMPLES)):
    ...
    #         if str(env.objects).count("GreenBall") == 1 and str(env.objects).count("PurpleBall") == 1:
    #             print(env.get_ram()[116])
    ...
    #             objects_infos[f"{obj_name}_x"].append(obj.xy[0])
...
cnts = [len(np.unique(r, return_counts=True)[1]) for r in ram_saves]
