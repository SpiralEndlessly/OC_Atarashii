...
def append_oinfo_values(obj, object_infos, objects_correctly_detected):
    ...
    # print(name)
    if (
        f"{name}_x" in object_infos.keys()
        and f"{name}_x" not in objects_correctly_detected
    ):
...
def generate_dataset(
    env, drop_constants, frames=700, skip_frames=3, manipulated_ram=None, start_frame=0
):
    """
...
def do_analysis(
    ...
    if (not os.path.exists(cand_file)) or new_dump:
        ...
        for obj_name in objects:
            ...
            candidates[obj_name] = {
                k: v
                for k, v in candidates[obj_name].items()
                if abs(v) > min_correlation
            }
            ...
            if len(candidates[obj_name]) > 1:
                for ram_pos in candidates[obj_name]:
                    ...
                    dataset2 = generate_dataset(
                        ...
                        drop_constants,
                        frames=300,
                        manipulated_ram=int(ram_pos),
                        start_frame=start_frame,
                    )
                    if obj_name in dataset2:
                        dataset2[obj_name] = np.array(dataset2[obj_name])

                        if len(dataset2[ram_pos]) > 0 and not (
                            np.all(dataset2[ram_pos] == dataset2[ram_pos][0])
                            or np.all(dataset2[obj_name] == dataset2[obj_name][0])
                        ):
                            corr2 = np.corrcoef(dataset2[obj_name], dataset2[ram_pos])[
                                1
                            ][0]
                ...
                for k in candidates[obj_name]:
                    ...
                    approved_candidates[obj_name].append(
                        {
                            "pos": int(k),
                            "corr": candidates[obj_name][k],
                            "manipulated_corr": None,
                        }
                    )
    ...
    for cand, arr in best_candidates.items():
        ...
        if cand[len(cand) - 1] == "y":
            maximum = maximum_y
        for pos in arr:
            offset_string = calculate_offset(
                dataset[cand],
                dataset[str(pos)],
                candidates[cand][str(pos)],
                str(pos),
                maximum=maximum,
            )
...
if __name__ == "__main__":
    ...
    NEW_DUMP = (
        True  # if True creates new datasets and dumps it overwriting the previous ones
    )
    # the minimal correlation required for a ram value to be relevant for an object
    MIN_CORRELATION = 0.8
    DROP_CONSTANTS = (
        True  # if True does not consider not changing variables for objects
    )
